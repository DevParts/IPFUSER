...
internal class Scripts
{
	...
	public static readonly string INCLUDE_EXISTS_DATABASE_ENCRYPTION_KEY = "IF {0} EXISTS (SELECT database_id FROM sys.dm_database_encryption_keys WHERE database_id = {1})";
	...
	public static readonly string INCLUDE_EXISTS_AVAILABILITY_REPLICA = "IF {0} EXISTS (SELECT ar.replica_server_name FROM sys.availability_groups AS ag INNER JOIN sys.availability_replicas AS ar ON (ar.group_id = ag.group_id) WHERE (ar.replica_server_name = {1}) and (ag.name = {2}))";

	public static readonly string INCLUDE_EXISTS_AVAILABILITY_DATABASE = "IF {0} EXISTS (SELECT db.name FROM sys.availability_groups AS ag INNER JOIN sys.dm_hadr_database_replica_states AS dbrs ON (dbrs.group_id = ag.group_id) AND (dbrs.is_local = 1) INNER JOIN sys.databases AS db ON db.database_id = dbrs.database_id WHERE (db.Name = {1}) AND (ag.Name = {2}))";

	public static readonly string INCLUDE_EXISTS_AVAILABILITY_GROUP_LISTENER = "IF {0} EXISTS (SELECT agl.dns_name FROM sys.availability_groups AS ag INNER JOIN master.sys.availability_group_listeners AS agl ON (agl.group_id = ag.group_id) WHERE agl.dns_name = {1})";
