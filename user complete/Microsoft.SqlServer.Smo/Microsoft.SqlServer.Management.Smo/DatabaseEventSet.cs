...
{
	public override int NumberOfElements => 149;
	...
	public DatabaseEventSet(params DatabaseEvent[] events)
	{
		base.Storage = new BitArray(NumberOfElements);
		foreach (DatabaseEvent bit in events)
		{
			SetBit(bit);
		}
	}
	...
	{
		return new DatabaseEventSet(base.Storage);
	}
	...
	public static DatabaseEventSet operator +(DatabaseEventSet eventSet, DatabaseEvent anEvent)
	{
		DatabaseEventSet databaseEventSet = new DatabaseEventSet(eventSet);
		databaseEventSet.SetBit(anEvent);
	...
	{
		DatabaseEventSet databaseEventSet = new DatabaseEventSet(eventSet);
		databaseEventSet.ResetBit(anEvent);
		return databaseEventSet;
	}
	...
	public override string ToString()
	{
		...
		foreach (bool item in base.Storage)
		{
			if (item)
			{
				...
				stringBuilder.Append(((DatabaseEventValues)num).ToString());
			}
			num++;
		}
