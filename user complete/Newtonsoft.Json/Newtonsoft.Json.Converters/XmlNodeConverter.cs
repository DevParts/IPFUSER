...
public class XmlNodeConverter : JsonConverter
{
	...
	{
		string text = (EncodeSpecialCharacters ? XmlConvert.EncodeLocalName(propertyName) : XmlConvert.EncodeName(propertyName));
		string prefix = MiscellaneousUtils.GetPrefix(text);
		...
		{
			if (childNode is IXmlElement xmlElement2 && xmlElement2.LocalName == text)
			{
		...
		do
		{
			...
			case JsonToken.PropertyName:
			{
				...
				if (reader.TokenType == JsonToken.StartArray)
				{
					...
					MiscellaneousUtils.GetQualifiedNameParts(EncodeSpecialCharacters ? XmlConvert.EncodeLocalName(text) : XmlConvert.EncodeName(text), out string prefix, out string localName);
