...
[ComVisible(false)]
internal class SqlObjectBase : EnumObject, ISqlFilterDecoderCallback
{
	...
	protected string QueryHint => m_QueryHint;
	...
	public virtual Assembly ResourceAssembly => Assembly.GetExecutingAssembly();

	public bool SupportsParameterization => ServerConnection.ParameterizationMode >= QueryParameterizationMode.ForcedParameterization;
	...
	private void AddQueryHintToResult(SqlEnumResult result)
	{
		...
		{
			result.AddQueryHint(m_QueryHint);
		}
	}
	...
	internal EnumResult BuildResult(EnumResult result)
	{
		...
		AddQueryHintToResult(sqlEnumResult);
		...
		if (base.Request.ResultType == ResultType.Default)
		{
		...
		{
			bool flag = false;
			foreach (DictionaryEntry postProcess in StatementBuilder.PostProcessList)
			{
				if (!(postProcess.Value is PostProcessCreateSqlSecureString) && (!(postProcess.Value is PostProcessBodyText) || ((ServerConnection)base.ConnectionInfo).ServerVersion.Major < 9))
				{
					flag = true;
					break;
				}
		...
		stopwatch.Stop();
		EnumResult enumResult = new EnumResult(ob, resultType);
		enumResult.CommandElapsedTime = stopwatch.Elapsed;
		enumResult.CommandText = sqlEnumResult.CommandText;
		return enumResult;
	}
	...
	protected object FillDataWithUseFailure(SqlEnumResult sqlresult, ResultType resultType)
	{
		...
		try
		{
			...
			sqlresult.CommandText = stringCollection.Cast<string>().Aggregate((string current, string next) => sb.AppendLine(next).ToString());
	...
	protected virtual object FillData(ResultType resultType, StringCollection sql, object connectionInfo, StatementBuilder sb)
	{
		...
		default:
			throw new ResultTypeNotSupportedEnumeratorException(resultType);
		}
	}
