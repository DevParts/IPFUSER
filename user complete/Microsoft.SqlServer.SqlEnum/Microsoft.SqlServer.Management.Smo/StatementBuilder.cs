...
internal class StatementBuilder
{
	...
	public string SqlStatement
	{
		get
		{
			...
			foreach (KeyValuePair<string, string> item in SqlEnumStatementBuilderTestHook.SqlStatementFragmentsToBeReplaced)
			{
	...
	internal StringBuilder InternalSelect()
	{
		...
		if (m_topN > 0)
		{
			stringBuilder.Append(string.Format(CultureInfo.InvariantCulture, "TOP ({0}) ", new object[1] { m_topN }));
		}
